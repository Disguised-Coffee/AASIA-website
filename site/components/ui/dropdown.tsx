...
const DropdownMenuTrigger = React.forwardRef<HTMLButtonElement, { children: React.ReactNode, onClick: () => void }>(
  ({ children, onClick }, ref) => (
    <button
      type="button"
      className="transition-colors duration-300 ease-in-out flex items-center px-4 py-2 text-lg bg-transparent hover:bg-white/20 md:text-gray-700 md:hover:text-gray-900 hover:text-white text-lg rounded" // dark:text-white transition-colors duration-200 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2"
      onClick={onClick}
      ref={ref}
    >
      {children}
