...
// Optional descriptions map to avoid ReferenceError
...
export default function GalleryPageClient({ galleries, categories }: GalleryPageClientProps) {
  ...
  const galleryCounts: Record<string, number> = {
    ...
    "Featured Events": (galleries ?? []).filter(
      (g) => Boolean(g?.featured) || (Array.isArray(g?.images) && g!.images!.some((img) => Boolean(img?.isFeatured)))
    ).length,
  }
            ...
            {galleries.map((gallery) => (
              ...
              >
                <div className="relative aspect-video">
                  <Image
                    src={withParams(gallery.images?.[0]?.asset?.url, "w=600&h=400&fit=crop&auto=format")}
